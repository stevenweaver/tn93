...
int main (int argc, const char * argv[]) {
    ...
    while (fasta_result == 2) {
        ...
        if (args.data == protein) 
          handle_a_sequence_aa (sequences, freq_counts, firstSequenceLength, copy_count.value(0), args.ambig == argparse::ignore);        
        else
          handle_a_sequence (sequences, freq_counts, firstSequenceLength, copy_count.value(0), args.ambig == argparse::ignore);
        sequences_read ++;
